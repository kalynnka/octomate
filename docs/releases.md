...
Octomate uses one Release Please release PR, one changelog, and one `vX.Y.Z` tag
for three PyPI distributions: `octomate-protocol`, `octomate-cli`, and `octomate`.
Their versions and internal dependency pins move together. The CLI remains
installable without the server.
...
Pushes to `main` update the release PR. Release Please updates the root version,
the annotated version and dependency lines in the workspace projects, and
`CHANGELOG.md`. The release workflow then runs `uv lock` on the release PR branch
...
The version manifest starts at `0.0.1`. Release Please calculates the next version
from Conventional Commits, using the same pre-1.0 bump policy as Arcanus. There is
no need to create or push a version tag manually.

After the release PR is merged:

1. Release Please creates the GitHub release and tag.
2. Checks run against that exact tag on Python 3.12 and 3.13. They run the test
...
3. The installation check rebuilds every source distribution, confirms internal
   dependency versions, verifies the client has no server dependencies, and runs
   the installed server with a disposable database and authenticated MCP check.
4. The publishing job uploads protocol, CLI, then server to PyPI and attaches all
   six distribution files to the GitHub release.
...
GitHub release automatically, fetches its tag, and exits unchanged when already
