...
flowchart TD
    ...
    U5["UoW-5 ✅<br/>channel.consume() dispatcher<br/>(drives present_output; rest deferred)"]
...
| **5** ✅ | `channel.consume()` dispatcher | 1, 3 | **Landed** in [channel/base.py](../../octomate/tentacles/channel/base.py): `ChannelTentacle.consume(key, stream)` drains the typed stream via a filtering generator → the answer (`ResultTextDeltaEvent`/`ResultSegmentEvent`/`FinalResult`) feeds `markdown_stream.present_output`; `Todo*`/`Ask`/`Approval`/passthrough hit explicit deferred `case`s (filled in UoW-7/8). **No production caller yet** (UoW-9 wires it into triage). Drives the stream-consuming `present_output` rather than the §4 per-event sketch (superseded by UoW-6). | 🟢 low |
