 ...
 * dossier, and the mock turn scripts. Real send/stream endpoints replace the
 * scripts in `sendDirective` / `sendNewThread` / `sendReview` when they exist.
 */
...
export interface ConsoleActions {
  ...
  startNewThread(): void
  setNtRoute(patch: Partial<Pick<ConsoleState, 'ntAgent' | 'ntModel' | 'ntEffort'>>): void
  ...
  const actions: ConsoleActions = {
    ...
    async selectThread(chId: string, thId: string) {
      ...
      set((s) => ({
        ...
        ntMenu: null,
        ntStarted: false,
        surface: ['dev_ui', 'slack', 'lark', 'napcat'].includes(chId) ? chId : s.surface,
        mgmtSec: '',
        pvOpen: false,
    ...
    setNtRoute(patch: Partial<Pick<ConsoleState, 'ntAgent' | 'ntModel' | 'ntEffort'>>) {
      set(patch)
    },
