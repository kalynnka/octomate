...
function CapsLabel({ children, style }: { children: ReactNode; style?: CSSProperties }) {
  return <span style={{ ...label(9), color: 'var(--fg-3)', ...style }}>{children}</span>
}
  ...
  return (
    <div style={{ border: '1px solid var(--line-divider)', margin: '0 0 10px' }}>
      ...
      >
        ...
        <span style={{ ...label(8), color: 'var(--fg-2)' }}>diff</span>
...
function CodeRow({ item, cardMax }: { item: Extract<LedgerItem, { kind: 'code' }>; cardMax: string }) {
  ...
  return (
    <div id={`pm-${item.uid}`} style={{ maxWidth: cardMax, border: '1px solid var(--line-divider)' }}>
      ...
      >
        <Disclose open={open} style={{ ...mono(10), ...ghost3, width: 11 }} />
        <span style={{ ...label(8), color: 'var(--fg-2)' }}>{item.lang}</span>
        <span style={{ flex: 1 }} />
        <span style={{ ...label(8), color: 'var(--fg-3)' }}>{item.lines.length} ln</span>
      </div>
      <Fold open={open}>
        <pre
          style={{
  ...
  return (
    ...
    >
      <div onClick={() => toggleCardOpen(item.uid, true)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', cursor: 'pointer' }}>
        ...
        <span style={{ ...label(10), color: 'var(--fg-1)' }}>Thinking</span>
        <span style={{ flex: 1 }} />
        <span style={{ ...label(9), color: 'var(--fg-3)' }}>{item.dur}</span>
      </div>
      <Fold open={open}>
  ...
  return (
    ...
    >
      <div onClick={() => toggleCardOpen(item.uid, true)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', cursor: 'pointer' }}>
        ...
        <span style={{ ...label(10), color: 'var(--fg-1)' }}>Plan</span>
        <span style={{ flex: 1 }} />
        <span style={{ ...label(9), color: 'var(--fg-3)' }}>{counter}</span>
      </div>
      <Fold open={open}>
...
function ToolDetailView({ detail }: { detail: ToolDetail }) {
  const heading = (t: string) => <span style={{ ...label(8, '.18em'), color: 'var(--fg-3)' }}>{t}</span>
    ...
    return (
      <div style={{ borderTop: '1px solid var(--line-divider)', padding: '10px 14px 11px' }}>
        ...
        >
          {detail.rows.map(([k, v]) => (
            <Fragment key={k}>
              <span style={{ ...label(8, '.16em'), color: 'var(--fg-3)', paddingTop: 3 }}>{k}</span>
    ...
    return (
      <>
        <div style={{ borderTop: '1px solid var(--line-divider)', padding: '10px 14px 11px' }}>
          ...
          >
            <span style={{ ...label(8, '.16em'), color: 'var(--fg-3)', paddingTop: 3 }}>repo</span>
            <span style={{ ...mono(11.5), color: 'var(--fg-1)' }}>"maple/staging"</span>
            <span style={{ ...label(8, '.16em'), color: 'var(--fg-3)', paddingTop: 3 }}>ref</span>
      ...
      <div style={{ borderTop: '1px solid var(--line-divider)', padding: '10px 14px' }}>
        ...
        {detail.spill && (
          <div style={{ marginTop: 9, padding: '8px 10px', border: '1px solid var(--color-gold)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ ...label(8, '.16em'), color: 'var(--color-gold)' }}>{detail.spill.label}</span>
  ...
  return (
    <div style={{ borderTop: '1px solid var(--line-divider)', padding: '10px 14px 12px' }}>
      ...
      >
        {detail.rows.map(([k, v]) => (
          <Fragment key={k}>
            <span style={{ ...label(8, '.16em'), color: 'var(--color-terra)', paddingTop: 3 }}>{k}</span>
            <span style={{ ...mono(11.5), color: 'var(--fg-1)' }}>{v}</span>
          </Fragment>
        ))}
      </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ ...label(10), color: 'var(--fg-1)' }}>Tool</span>
        ...
        {item.status === 'run' ? (
          <>
            ...
            <span style={{ ...label(8, '.18em'), color: 'var(--color-accent)' }}>run</span>
          </>
        ) : (
          <>
            <span style={{ width: 5, height: 5, borderRadius: 9999, background: 'var(--color-sage)' }} />
            <span style={{ ...label(8, '.18em'), color: 'var(--color-sage)' }}>done</span>
          </>
        )}
      </div>
      <Fold open={open}>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px' }}>
        ...
        <span style={{ flex: 1 }} />
        {isOpen ? (
          <span style={{ ...label(7.5, '.14em'), color: 'var(--color-accent)', flexShrink: 0 }}>● open</span>
        ) : (
    ...
    <div id={`pm-${item.uid}`} className="lt-entry" style={{ '--i': i ?? 0, maxWidth: cardMax } as CSSProperties}>
      <div style={{ border: '1px solid var(--line-divider)', borderLeft: '3px solid var(--color-gold)', background: 'var(--card-bg)', padding: '11px 15px 11px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          ...
          <span style={{ flex: 1 }} />
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 1, ...label(7.5, '.16em'), color: 'var(--color-gold)', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 10, flexWrap: 'wrap' }}>
          <span
            ...
            className="hov-panel"
            style={{ ...label(8.5, '.12em'), color: '#F6F1E5', background: 'var(--color-accent)', padding: '4px 10px', cursor: 'pointer' }}
          >
            Approve
          </span>
        ...
        <div style={{ marginTop: 9, ...label(7.5, '.14em'), color: 'var(--fg-3)' }}>{item.meta}</div>
      </div>
    </div>
  )
}
      ...
      <div id={`pm-${item.uid}`} className="lt-entry" style={{ '--i': i ?? 0, maxWidth: cardMax } as CSSProperties}>
        {!ansOpen ? (
          ...
          <div className="lt-fade-in" style={{ border: '1px solid var(--line-divider)', borderLeft: '3px solid var(--color-sage)', background: 'var(--card-bg)', padding: '9px 12px 10px' }}>
            ...
            <div style={{ marginTop: 8, ...label(7.5, '.14em'), color: 'var(--fg-3)' }}>{item.via} · RUN_0521 resumed</div>
          </div>
        )}
      </div>
    )
      ...
      <div style={{ border: '1px solid var(--line-divider)', borderLeft: '3px solid var(--color-gold)', background: 'var(--card-bg)', paddingTop: 12 }}>
        <div style={{ padding: '0 15px 0 16px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
            ...
            <span style={{ flex: 1 }} />
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 2, ...label(8, '.16em'), color: 'var(--color-gold)', whiteSpace: 'nowrap' }}>
            ...
            {otherOpen && (
              <div className="lt-fade-in" style={{ padding: '0 15px 10px 16px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                  <span style={{ ...label(7.5, '.14em'), color: 'var(--fg-3)' }}>↩ send · esc back to options</span>
                  <span style={{ flex: 1 }} />
                  <span onClick={sendOther} className="hov-panel" style={{ ...label(8.5, '.12em'), color: '#F6F1E5', background: 'var(--color-accent)', padding: '4px 10px', cursor: 'pointer' }}>
                    Send
                  </span>
                </div>
              </div>
        ...
        <div style={{ padding: '8px 15px 10px 16px', ...label(7.5, '.14em'), color: 'var(--fg-3)' }}>{item.meta}</div>
      </div>
    </div>
  )
}
...
export function LedgerRow({ item, cardMax, i }: { item: LedgerItem; cardMax: string; i?: number }) {
  switch (item.kind) {
    case 'divider':
      return (
        <Rule>
          <span style={{ ...label(8, '.18em'), color: 'var(--fg-3)' }}>{item.label}</span>
        </Rule>
      )
    case 'system':
      return (
