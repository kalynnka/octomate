        ...
        {open && (
          ...
          >
            <span style={{ display: 'flex', alignItems: 'baseline', gap: 8, padding: '8px 12px 7px', borderBottom: '1px solid var(--line-divider)' }}>
              <span style={{ ...label(8), color: 'var(--fg-2)' }}>Agent</span>
              <span style={{ flex: 1 }} />
            ...
            {agentRoutes.map((ar) => {
              ...
              return (
                <span
                  key={ar.name}
                  onClick={() => {
                    if (!on) setNtRoute({ ntAgent: ar.name, ntModel: ar.models[0] })
                  }}
                  className="hov-wash"
                  style={{
                    display: 'flex',
                ...
                >
                  ...
                  <span style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 3 }}>
                    <span style={{ display: 'flex', alignItems: 'baseline', gap: 7 }}>
                      ...
                      <span style={{ ...mono(8), color: 'var(--color-accent)' }}>{on ? ntModel : ar.models[0]}</span>
                    </span>
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 12px', background: 'var(--surface-sunken)' }}>
              <span style={{ ...label(8), color: 'var(--fg-2)' }}>Effort</span>
...
export function Composer() {
  ...
  const ntEffort = useConsole((s) => s.ntEffort)
  const ntMenu = useConsole((s) => s.ntMenu)
  const { removeQueued, closeNtMenu } = useConsole((s) => s.actions)
  const { data: channels } = useChannels()

  const isReview = selThreadId === 'THR-0195'
  const lastSes = detail?.sessions[detail.sessions.length - 1]
  const routeChip = `${channels?.find((c) => c.id === selChannel)?.label.toLowerCase() ?? 'web'}/${selThreadId}`
      ...
      <div style={{ borderTop: '2px solid var(--trk-bracket)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '7px 24px 6px', borderBottom: '1px solid var(--trk-vline)' }}>
          ...
          <span
            ...
            style={{ ...label(7.5, '.16em'), color: 'var(--color-gold)', border: '1px solid var(--color-gold)', padding: '2px 6px', whiteSpace: 'nowrap' }}
          >
            MD·Live
          </span>
        </div>
        ...
        <div
          onClick={() => ntMenu === 'sel' && closeNtMenu()}
        ...
        >
          ...
          <span style={{ ...label(8), color: 'var(--color-accent)' }}>Directive</span>
          {queue.length > 0 && (
