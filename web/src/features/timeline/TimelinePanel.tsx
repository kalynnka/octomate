...
import { ellipsis, label, mono, serif } from '@/components/text'
...
function eventOf(item: LedgerItem): TlEvent | null {
  ...
  switch (item.kind) {
    ...
    case 'stream':
      return { k: 'msg', title: 'claude', sub: item.streaming ? 'reply streaming…' : item.text, t: '', tgt }
    default:
      return null
  }
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ flex: 1 }} />
        <span style={{ ...mono(8.5, 700), color: 'var(--color-accent)' }}>{selThreadId}</span>
      </div>
      <div
        style={{
          display: 'flex',
              ...
              <div style={{ borderBottom: '1px solid var(--line-divider)' }}>
                ...
                {folded && (
                  <div
                    ...
                    className="hov-accent"
                    style={{ padding: '4px 12px 5px 33px', ...label(7.5, '.16em'), color: 'var(--fg-3)', cursor: 'pointer' }}
                  >
