"""Slack's own tools served at `/slack/mcp`, as the person who drove the turn.
...
CONNECT_TOOL = SlackTentacle.connect_tool()
CONFIRM_TOOL = SlackTentacle.confirm_tool()
...
async def connected(app: FastAPI, ink: FakeSlackInk, client: Client) -> None:
    ...
    result = await client.call_tool(CONNECT_TOOL, {})
    assert result.data == (
...
@asynccontextmanager
async def in_memory(
    session: GatewaySession, transport: httpx.AsyncBaseTransport
) -> AsyncIterator[Client]:
    """The Slack server mounted for one fixed turn, its upstream `transport`."""

    async def fixed() -> GatewaySession:
        return session

    async with Client(
        SlackTentacle.mcp(fixed, httpx_client_factory=into(transport))
    ) as client:
        yield client


def test_a_slack_workspace_adds_one_server_however_many_there_are() -> None:
    octomate = a_deployment()
    a_workspace(octomate, FakeSlackInk())
    a_workspace(octomate, FakeSlackInk(), id="slack-b")

    assert [server.name for server in octomate.mcp_servers()] == ["gateway", "slack"]


async def test_a_caller_with_no_turn_is_listed_only_the_link_tools() -> None:
    octomate = a_deployment()
    a_workspace(octomate, FakeSlackInk())
    async with served(octomate) as (octomate, app):
        async with over(octomate, app, BEARER, server="slack") as client:
...
async def test_a_call_outside_a_slack_turn_is_refused() -> None:
    ...
    async with served(octomate) as (octomate, app):
        ...
        async with over(octomate, app, naming(elsewhere), server="slack") as client:
        ...
        async with over(octomate, app, naming(nowhere), server="slack") as client:
            with pytest.raises(ToolError, match="nowhere in Slack to act"):
                await client.call_tool(CONNECT_TOOL, {})
...
async def test_an_unconnected_caller_is_listed_nothing_and_told_to_connect() -> None:
    ...
    async with served(octomate) as (octomate, app):
        ...
        async with over(octomate, app, naming(session), server="slack") as client:
            tools = await client.list_tools()
            with pytest.raises(ToolError, match=f"`{CONNECT_TOOL}`"):
                await client.call_tool("slack_read_user_profile", {})
            status = await client.call_tool(CONFIRM_TOOL, {})
...
async def test_a_connected_caller_is_listed_slacks_tools_and_acts_as_themselves() -> (
    ...
    async with served(octomate) as (octomate, app):
        ...
        async with (
            ...
            in_memory(session, httpx.ASGITransport(app=upstream_app)) as client,
        ):
            tools = await client.list_tools()
            result = await client.call_tool("slack_read_user_profile", {})

    # The list is Slack's, fetched for this person, and the call reached Slack
    # as them, with the token Octomate holds — which the driven runtime never saw.
...
async def test_a_token_slack_has_revoked_retires_the_connection() -> None:
    ...
    async with served(octomate) as (octomate, app):
        ...
        async with in_memory(session, refusing) as client:
