...
/**
 * Where this thread's work lives: the project it is filed under when one claims
 * its directory, and the directory itself either way. A native session that no
 * project holds still has somewhere to open, so the strip renders on the
 * directory rather than on the filing.
 */
export function ProjectStrip() {
  const project = useConsole((s) => s.detail?.project)
  const vscode = useConsole((s) => s.detail?.vscode)
  const vsLaunch = useConsole((s) => s.vsLaunch)
  const { vsOpen } = useConsole((s) => s.actions)
  const dir = vscode?.dir ?? project?.path
  if (!dir) return null
  return (
    <div
      id="trk-projstrip"
    ...
    >
      <span
        style={{
          ...mono(11, 700),
          color: project ? 'var(--color-accent)' : 'var(--fg-3)',
          lineHeight: 1,
        }}
      >
        ⌗
      ...
      {project ? (
        ...
        <span
          title="no project claims this directory — the session ran here all the same"
          style={{ ...label(8, '.12em'), color: 'var(--fg-3)', whiteSpace: 'nowrap', flexShrink: 0 }}
        >
          no project
        </span>
      )}
      <span style={{ ...mono(8.5), color: 'var(--fg-3)', ...ellipsis, minWidth: 0, flexShrink: 1 }}>{dir}</span>
      {project?.cwd && (
        <span
      ...
      {vscode && (
        <span
          ...
          title={
            vscode.session
              ? `Open ${dir} in VS Code and reopen this session in its extension`
              : `Open ${dir} in VS Code`
          }
