...
channel itself, and other channels' threads are read-only views. In production there is a single entry —
`octomate service serve` serves the enabled Trunkline channel's
`static_dir` at `/` alongside the API. Build with `pnpm build`, then set
`static_dir` in `channels.yaml` to the compiled `dist/` directory. Leaving it unset
keeps the console API available without static files; the Vite dev server is only
for HMR.

Use `octomate service serve --reload` for backend development. This runner starts
tentacles after the API listener opens and stops them before it closes, so runtime
hooks can be delivered during both phases. A direct Uvicorn factory launch serves
only the API; it does not enter tentacle contexts.
