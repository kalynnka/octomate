...
One module per capability — ask, send, todos, history, gateway, mcp — each
...
the deferred-tool protocols) lives in `harness`.
...
mounts the few it needs, and reaching one should not pull the whole set into the
importer's graph. What this package does define is the one contract a capability
can opt into beyond pydantic-ai's own.
"""
